...
const SuggestionItem = ({ product, onClick }) => (
  <li onClick={onClick}>
    <Link
      to={`/productos/${
        product.productoId || product.productid || product.productoid
      }`}
    ...
    <header className="sticky top-0 z-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="shrink-0">
              <Link to="/" className="text-2xl font-serif text-[#0A0A0A]">
                El Zar
              </Link>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link
                to="/carrito"
                className="relative text-[#6E6E6E] hover:text-[#D4AF37] transition-colors duration-200"
              >
                <span className="sr-only">Cart</span>
                <FiShoppingCart className="w-5 h-5" />
                {totalItemsInCart > 0 && (
                  <span className="absolute -top-2 -right-2 flex h-5 w-5 items-center justify-center rounded-full bg-[#D4AF37] text-xs font-bold text-[#0A0A0A]">
                    {totalItemsInCart}
                  </span>
                )}
              </Link>
      ...
      <div
        className={`absolute left-0 right-0 bg-[#F5F5F5] shadow-md transition-all duration-550 ease-in-out ${
          isSearchOpen
            ? "top-[calc(2.25rem+4rem)] opacity-100"
            : "-top-full opacity-0"
        }`}
      >
